...
- **Python 3.11+**
- **[LM Studio](https://lmstudio.ai/)** with a model that meets three requirements: tool use / function calling support, decent Brazilian Portuguese, and a context window of **20k tokens or more** (up from an earlier 16k recommendation — the system prompt alone is ~3.5k tokens plus ~1.5-2k for the 7 tool schemas, a fixed ~5.5k cost per turn before any conversation history or tool output; the margin at 16k has gotten thin as the prompt grew with more guardrails, even though the default `MAX_HISTORY_TOKENS=8000` still technically fits). The project does not depend on any specific model: `.env` allows swapping the model and endpoint without touching code, and was exercised across multiple model sizes throughout development. The model used for versioned evaluation is listed in the header of [`eval_report.md`](eval_report.md), which `test_live.py` automatically generates. Larger models handle tool calling more reliably and write better Portuguese at the expense of latency (see Decision 2). In LM Studio: max GPU offload recommended.
