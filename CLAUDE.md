...
| 5 | Histórico de conversa | Checkpointer **`SqliteSaver`** do LangGraph, por `thread_id` + poda com `trim_messages` no `pre_model_hook` | Persiste entre execuções, reaproveita o SQLite. Persistir tudo ≠ reenviar tudo: `MAX_HISTORY_TOKENS` (32000) evita estourar a janela do modelo local em conversa longa. |
